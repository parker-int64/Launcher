...
本章说明 APPLaunch 首页 UI 的组织方式、轮播卡片的数据流和按键事件。重点参考 `projects/APPLaunch/main/ui/UILaunchPage.cpp`、`projects/APPLaunch/main/ui/UILaunchPage.h`、`projects/APPLaunch/main/ui/Animation/ui_launcher_animation.cpp`、`projects/APPLaunch/main/ui/Launch.cpp`。
...
首页复用 `home_base` / `AppPageRoot` 页面框架创建根 screen、状态栏和输入 group。`UILaunchPage.cpp` 负责在继承来的内容容器中填充轮播，并绑定 LVGL 回调。

## 2. 关键源码路径

| 路径 | 说明 |
| --- | --- |
| `projects/APPLaunch/main/ui/UILaunchPage.h` | 首页类定义、轮播元素枚举、`carousel_elements` 数组 |
| `projects/APPLaunch/main/ui/UILaunchPage.cpp` | 首页 screen 创建、轮播切换、键盘事件、启动 GIF、字体缓存 |
...
- 实例职责：持有 `Launch` 指针，拥有页面级 UI 状态，处理 LVGL 事件，并把轮播更新和应用启动转发给 `LaunchImpl`。
...
`UILaunchPage.cpp` 中用 `CarouselSlot` 描述轮播静态布局：
...
状态栏数据刷新不在 `UILaunchPage`，而在 `LaunchImpl::update_home_status_bar()`：
...
默认标题只是 UI 初始占位，真正内容会由 `LaunchImpl` 初始化应用列表后写入。
...
`LaunchImpl` 维护 `current_app` 和 `app_list`。切换时，`UILaunchPage` 只传入要被复用的 panel/label；具体要显示哪个应用由 `LaunchImpl` 计算。
...
- 状态栏对象由 `UILaunchPage` 创建，但刷新 timer 在 `LaunchImpl` 构造时创建；如果只创建首页但没有执行 `Launch::bind_ui()`，应用列表和状态栏刷新不会启动。
